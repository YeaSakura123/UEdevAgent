...
class TodoManager:
    ...
    # 内部函数：初始化当前类实例，准备 todo 列表和持久任务图管理 所需状态。
    def __init__(self, agent_dir: Path):
        self.agent_dir = agent_dir
        self.path = agent_dir / "todos.json"
        self.agent_dir.mkdir(parents=True, exist_ok=True)

    # 外部函数：更新 todo 或任务状态数据，负责 todo 列表和持久任务图管理。
    ...
    # 外部函数：读取磁盘上的 todo 列表，供展示和 loop 状态判断使用。
...
class TaskManager:
    ...
    # 外部函数：实现 task_create 工具能力，创建带依赖、owner 和状态的任务文件。
    def create(
        self,
        subject: str,
        description: str = "",
    ...
    # 外部函数：读取指定任务数据，负责 todo 列表和持久任务图管理。
    ...
    # 外部函数：实现 task_list 和 CLI tasks --graph 展示，列出任务图摘要。
    ...
    # 外部函数：把任务标记为指定 owner 进行中，负责 todo 列表和持久任务图管理。
    ...
    # 内部函数：处理 _path 辅助逻辑，支撑 todo 列表和持久任务图管理。
    def _path(self, task_id: int) -> Path:
        return self.tasks_dir / f"task_{task_id}.json"

    # 内部函数：处理 _load 辅助逻辑，支撑 todo 列表和持久任务图管理。
