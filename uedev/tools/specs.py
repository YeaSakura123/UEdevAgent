...
def get_tool_specs(extra_tools: list[ToolSpec] | None = None) -> list[ToolSpec]:
    ...
    tools = [
        ...
        function_tool(
            "read_file",
            "Read a UTF-8 text file from the workspace, excluding configured internal/generated directories.",
            {"path": string, "limit": integer},
            ["path"],
        ),
        function_tool(
            "write_file",
            "Write UTF-8 text content to a workspace file, excluding configured internal/generated directories.",
            {"path": string, "content": string},
            ["path", "content"],
        ),
        function_tool(
            "edit_file",
            "Replace the first occurrence of old_text with new_text in a workspace file, excluding configured internal/generated directories.",
            {"path": string, "old_text": string, "new_text": string},
            ["path", "old_text", "new_text"],
        ),
        function_tool(
            "list_files",
            "List files under a workspace directory, skipping configured internal/generated directories.",
            {"path": string, "pattern": string, "limit": integer},
        ),
        function_tool(
            "grep",
            "Search workspace text content with a structured read-only grep tool. Prefer this over shell rg/grep for code and config searches. It supports regex pattern, optional path/glob filters, content/files/count output modes, and can include UE asset path/name matches without reading binary asset contents.",
            {
                "pattern": string,
                "path": string,
                "glob": string,
